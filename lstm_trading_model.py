...
class LSTMStockPredictor:
    ...
    """
    def __init__(self, ticker, seq_length=30, hidden_size=128, epochs=100, lr=0.0005):
    ...
    def create_sequences(self, data):
        ...
        for i in range(self.seq_length, len(data) - 3):
            ...
            direction.append(1 if data[i + 3, 0] > data[i, 0] else 0)  # Price direction after 3 days
    ...
    def train(self, X_train, y_price_train, y_dir_train):
        ...
        pos_weight = torch.tensor([(len(y_dir_train) - y_dir_train.sum()) / y_dir_train.sum()]).to(self.device)
        pos_weight = torch.clamp(pos_weight, max=3.0)  # Avoid over-penalization
        ...
        for epoch in range(self.epochs):
            ...
            for batch_X, batch_y_price, batch_y_dir in train_loader:
                ...
                pred_dir_logits = torch.clamp(pred_dir_logits, min=-5, max=5)  # Prevent extreme values
                loss_price = criterion_price(pred_price, batch_y_price)
                loss_dir = criterion_dir(pred_dir_logits, batch_y_dir)
                loss = loss_price + 0.5 * loss_dir  # Weighted combination of losses
    ...
    def evaluate(self, X_test, y_price_test):
        ...
        with torch.no_grad():
            ...
            # Padding second column to make scaler work (scaler expects 2 columns)
            dummy_pred = np.zeros((len(pred_price), 2))
            dummy_pred[:, 0] = pred_price[:, 0]
            dummy_true = np.zeros((len(true), 2))
            dummy_true[:, 0] = true[:, 0]

            pred_price = self.scaler.inverse_transform(dummy_pred)[:, 0]
            true = self.scaler.inverse_transform(dummy_true)[:, 0]
    ...
    def evaluate_accuracy(self, X_test, y_price_test, y_dir_test):
        ...
        with torch.no_grad():
            ...
            dummy_pred = np.zeros((len(pred_price_np), 2))
            dummy_pred[:, 0] = pred_price_np[:, 0]
            dummy_true = np.zeros((len(y_price_np), 2))
            dummy_true[:, 0] = y_price_np[:, 0]

            pred_price_rescaled = self.scaler.inverse_transform(dummy_pred)[:, 0]
            true_price_rescaled = self.scaler.inverse_transform(dummy_true)[:, 0]
...
if __name__ == "__main__":
    # Instantiate model
    predictor = LSTMStockPredictor("AAPL", seq_length=30, epochs=50)
